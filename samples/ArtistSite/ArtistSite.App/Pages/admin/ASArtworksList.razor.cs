...
namespace ArtistSite.App.Pages
{
	...
	public partial class ASArtworksListViewModel : AdminPageBase
	{
		public ASArtworksListViewModel()
		{
		}
		...
		protected Artwork SelectedArtwork { get; set; }

		[Inject]
		private IDialogService DialogService { get; set; }

		protected async Task ConfirmDeleteAsync(Artwork item)
		{
				...
				if (!result.Cancelled)
				{
						await DeleteArtworkAsync(item.ArtworkId);
				}
		}

		protected async Task DeleteArtworkAsync(int artworkId)
		{
				var result = await WebApiDataServiceAS.DeleteArtworkAsync(artworkId);
		...
		protected bool FilterArtwork1(Artwork item) => FilterFunc(item, SearchString1);

		protected bool FilterFunc(Artwork item, string searchString)
		{
		...
		protected override async Task OnInitializedAsync()
		{
				...
				try
				{
						List<BreadcrumbItem> breadcrumbs = new List<BreadcrumbItem>()
						{
								...
								new BreadcrumbItem("List of Artworks", "/admin/artworks", disabled: true)
						};
						...
						if (Artworks == null || !Artworks.Any())
						{
								if (User != null && User.Identity.IsAuthenticated)
								{
										...
										Artworks = await WebApiDataServiceAS.GetAllPagesArtworksAsync();
								}
						}
				}
				finally
