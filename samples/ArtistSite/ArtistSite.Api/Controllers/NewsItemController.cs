...
{
	public partial class NewsItemController : ASBaseApiController
	{
		private const string GET_LIST_ROUTE_NAME = "NewsItemASList";
		private IASGenericFactory<entAS.NewsItem, dtoAS.NewsItem> _factory;
		...
		public NewsItemController(ILogger<NewsItemController> logger,
			...
			IASGenericFactory<entAS.NewsItem, dtoAS.NewsItem> factory)
		...
		public async Task<IActionResult> Delete(int newsItemId)
		{
				try
				{
						...
						var result = await Repo.Delete_NewsItemAsync(newsItemId);
			...
			int page = 1, int pageSize = 100)
		{
				try
				{
						...
						RepositoryPageDataResponse<IList<entAS.NewsItem>> response =
							await Repo.GetPageData_NewsItemAsync(request);
		...
		[HttpGet(template: "ById/{newsItemId}/{relatedEntitiesType:relatedEntitiesType=None}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Get(int newsItemId, Enums.RelatedEntitiesType relatedEntitiesType)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						var dbItem = await Repo.Get_NewsItemAsync(newsItemId, relatedEntitiesType);
						if (dbItem == null)
							return PrepareNotFoundResponse();

						RunCustomLogicOnGetEntityByPK(ref dbItem, newsItemId, relatedEntitiesType);
		...
		[HttpPatch("{newsItemId}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Patch(int newsItemId, [FromBody] JsonPatchDocument<dtoAS.NewsItem> patchDocument)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						var dbItem = await Repo.Get_NewsItemAsync(newsItemId);
						if (dbItem == null)
							return NotFound();
		...
		[HttpPost]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Post([FromBody] dtoAS.NewsItem dtoItem)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						if (result.Status == RepositoryActionStatus.Created)
						{
								...
								var uriFormatted = LinkGenerator.GetUriByAction(
									...
									values: new { newDTOItem.NewsItemId }
									);
		...
		[HttpPut("{newsItemId}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Put(int newsItemId, [FromBody] dtoAS.NewsItem dtoItem)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						RunCustomLogicBeforeUpdatePut(ref updatedDBItem, newsItemId);
						var result = await Repo.UpdateAsync(updatedDBItem);
