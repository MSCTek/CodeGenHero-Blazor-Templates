...
{
	public partial class ArtworkController : ASBaseApiController
	{
		private const string GET_LIST_ROUTE_NAME = "ArtworkASList";
		private IASGenericFactory<entAS.Artwork, dtoAS.Artwork> _factory;
		...
		public ArtworkController(ILogger<ArtworkController> logger,
			...
			IASGenericFactory<entAS.Artwork, dtoAS.Artwork> factory)
		...
		public async Task<IActionResult> Delete(int artworkId)
		{
				try
				{
						...
						var result = await Repo.Delete_ArtworkAsync(artworkId);
			...
			int page = 1, int pageSize = 100)
		{
				try
				{
						...
						RepositoryPageDataResponse<IList<entAS.Artwork>> response =
							await Repo.GetPageData_ArtworkAsync(request);
		...
		[HttpGet(template: "ById/{artworkId}/{relatedEntitiesType:relatedEntitiesType=None}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Get(int artworkId, Enums.RelatedEntitiesType relatedEntitiesType)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						var dbItem = await Repo.Get_ArtworkAsync(artworkId, relatedEntitiesType);
						if (dbItem == null)
							return PrepareNotFoundResponse();

						RunCustomLogicOnGetEntityByPK(ref dbItem, artworkId, relatedEntitiesType);
		...
		[HttpPatch("{artworkId}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Patch(int artworkId, [FromBody] JsonPatchDocument<dtoAS.Artwork> patchDocument)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						var dbItem = await Repo.Get_ArtworkAsync(artworkId);
						if (dbItem == null)
							return NotFound();
		...
		[HttpPost]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Post([FromBody] dtoAS.Artwork dtoItem)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						if (result.Status == RepositoryActionStatus.Created)
						{
								...
								var uriFormatted = LinkGenerator.GetUriByAction(
									...
									values: new { newDTOItem.ArtworkId }
									);
		...
		[HttpPut("{artworkId}")]
		[VersionedActionConstraint(allowedVersion: 1, order: 100)]
		public async Task<IActionResult> Put(int artworkId, [FromBody] dtoAS.Artwork dtoItem)
		{
				try
				{
						if (!base.OnActionExecuting(out int httpStatusCode, out string message))
						...
						RunCustomLogicBeforeUpdatePut(ref updatedDBItem, artworkId);
						var result = await Repo.UpdateAsync(updatedDBItem);
